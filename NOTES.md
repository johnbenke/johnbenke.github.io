...
and readout — the look of a 1960s service manual or spec sheet, chosen to suit
the restoration work that now leads the page. Deliberately quiet: no texture, no
ornament, nothing that competes with the photographs.

Cards carry plate numbers (01–08) generated by a CSS counter from the grid
order, so reordering the cards renumbers them automatically.
